...
def check_group_exists_or_create():
    url = f"https://io.adafruit.us/api/v2/{config.adafruit_io_username}/groups"
    headers = {
...
def fetch_time(synch_with_rtc=True, timeout=15, retry=True):
    ...
    try:
        ...
        url = f"https://io.adafruit.us/api/v2/{config.adafruit_io_username}/integrations/time/struct?tz=UTC"
        headers = {
            "X-AIO-Key": config.adafruit_io_key,
            "Content-Type": "application/json",
        }
...
def upload_reading(reading):
    ...
    url = f"https://io.adafruit.us/api/v2/{username}/groups/{group_name}/data"
