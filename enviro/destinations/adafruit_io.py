...
def check_group_exists_or_create():
    ...
    logging.info(f"creating group using url {url} with payload:\n{payload}")
    try:
        ...
        try:
            ...
            if not error_message in ("", None, []):
                logging.error(
                    f"An error occurred with the group creation request: {"".join(error_message)}"
                )
                result.close()
                return False
        ...
        logging.info(f"  - an exception occurred when creating group.", buf.getvalue())
        return False
    ...
    try:
        ...
        try:
            ...
            if result.status_code != 200:
                logging.info(
                    f"  - fetch issue '{error_message}' ({result.status_code}"
                )
                logging.info(f" {url} returned {timestamp}")
...
def upload_reading(reading, create_group=True):
    ...
    try:
        result = urequests.post(url, json=payload, headers=headers)
        logging.debug(f"got a {result.status_code} result from upload attempt")
        error_message = ""
        try:
            error_message = result.json()["error"]
        except (TypeError, KeyError):
            # as te:
            # logging.debug(f"type/key error: {te}")
            pass
        except(ValueError, OSError) as esc:
            logging.info(f"  - upload issue '{esc}'")
            logging.info(f"- response text: '{result.text}'")
            result.close()
            return UPLOAD_FAILED
        result.close()
        if result.status_code == 429:
            logging.info("Rate limited during upload")
        ...
        if result.status_code in [400, 404]:  # bad request / group not found
            logging.info(f"  - upload got 404: {error_message}")
            if error_message.find("not found - There is no feed with the key")>=0:
                logging.info(
                    f"  - upload issue '{"".join(error_message)}' - Possibly missing group '{group_name}' - will attempt to create it"
                )
                try:
                    if check_group_exists_or_create():
                        return upload_reading(reading, create_group=False)
                    else:
                        return UPLOAD_FAILED
                except Exception as exc:
                    logging.info(
        ...
        # I get dns failures early on after connecting which throws here
        if ose.errno == -2:
            logging.debug("  - initial OSError, waiting and retrying")
            time.sleep(3)  # instead maybe await ping to check dns then retry
            return upload_reading(reading)
        else:
            logging.error(f"  - upload issue '{ose}'")
    except Exception as exc:
        import sys, io

        buf = io.StringIO()
        sys.print_exception(exc, buf)
        logging.info(f"  - an exception occurred when uploading.", buf.getvalue())
